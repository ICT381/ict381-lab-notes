...
## Pre-requisites
1. Completed all the tasks in LAB_5A_MAC and LAB_5B_MAC.
...
**Configuration of SSH credential for jenkins user**
1. Create SSH credential for the jenkins user
2. Configure Ansible files

**Configuration for Jenkins**
1. Add SSH key to Jenkins to access GitHub repository
2. Add DockerHub credentials to Jenkins
3. Save and push changes to the automation repository
4. Create the Jenkins pipeline
5. Run the Jenkins pipeline
6. Create and configure a GitHub webhook
7. Configure Jenkins to trigger on push
...
Connect to your Jenkins machine using SSH from your terminal and then perform the steps below.
...
3. Create the `tag_group_web.yaml` file in the `/etc/ansible/group_vars` directory.
...
1. Continuing from the previous task, click **+ Add Credentials** button.
...
## Task 5: Creating the Jenkins Pipeline
...
7. Under **Repository URL**, enter your Git repository in format:
 `git@github.com:GIT_USERNAME/automation`.

8. Under **Credentials**, select **jenkins** from the dropdown list.

9. Under the branch specifier, enter the branch name: `*/main`.

    ![](../images/lab5C/pipeline-settings.png)

10. Leave the rest as default and scroll down to the end of page and click **Save**.
...
*  Open `App.js` in `myReactApp\src\App.js`.
*  On line 27, introduce a change. Replace **STX** with **STX1** inside the Link tag.
*  Save and push the changes back to your repository.
*  Verify that a new build starts automatically in Jenkins.
*  Refresh the application in your browser to confirm the change is live.
