...
2. Run Unit testing
3. Run Feature testing


## Task 1: Creating an .env file

In line 9 of the `tests/conftest.py` file, the `load_dotenv()` function from the `dotenv` library is used to import all environment variables into the system. This affects the behavior of the application. For instance, the `create_app()` function checks the value of the `FLASK_ENV` environment variable:
...
echo 'FLASK_ENV=development' > ~/StaycationX/.env
```
...
You have completed this lab exercise. Do move on to the next lab exercise for selenium testing.