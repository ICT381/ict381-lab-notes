...
This lab continues from **LAB_3C Task 3**. Complete all steps on your development machine.
...
4. The `myReactApp` production build is already copied to `/usr/share/nginx/html` from LAB_3C.
...
2. Run the following command to generate a certificate and private key:
   ...
   - `/etc/nginx/ssl/localhost.crt` (certificate)
   - `/etc/nginx/ssl/localhost.key` (private key)

## Task 2: Update Nginx for HTTPS

From LAB_3C, you have already:
...
Now replace the default site configuration with an HTTPS server block.

1. Replace `/etc/nginx/sites-available/default`:
...
2. Understand the key directives:
   ...
   - `location /` with `try_files`: Serves static files from the `/usr/share/nginx/html`. The `try_files` directive first checks for the requested path. If no matching file exists, it returns `/index.html`. This fallback is required for React single-page app routing.
