...
if [[ -z "$GIT_USERNAME" || "$GIT_USERNAME" == "XXX" ]]; then
    echo "ERROR: GIT_USERNAME is not set. Edit the script and replace 'XXX' with your GitHub username."
    exit 1
fi

# Check git is installed
if ! command -v git &>/dev/null; then
    echo "ERROR: git is not installed or not in PATH. Aborting."
    exit 1
fi
